...
_SELECTED_DOMAIN_KEY = "selected_domain"
_SELECTED_CATEGORY_KEY = "selected_category"
_SELECTED_STRATEGY_KEY = "selected_strategy"
...
class StrategyDiscoveryTools(Toolkit):
    ...
    def select_analysis_domain(
        ...
        You MUST call this after deciding the domain, before exploring
        strategy categories.

        Args:
            domain: The domain id, e.g. "technical analysis".
        """
        try:
        ...
        run_context.session_state[_SELECTED_DOMAIN_KEY] = chosen.value
        run_context.session_state.pop(_SELECTED_CATEGORY_KEY, None)
        run_context.session_state.pop(_SELECTED_STRATEGY_KEY, None)
    ...
    def list_strategy_categories(
        ...
        Returns:
            A list of category descriptors with summary, use_if, example_queries.
        """
        domain_value = run_context.session_state.get(_SELECTED_DOMAIN_KEY)
    ...
    def select_strategy_category(
        ...
        """
        Record which strategy category you have chosen. Must be called
        after select_analysis_domain.

        Args:
            category: The category id, e.g. "momentum", "trend".
        """
        if _SELECTED_DOMAIN_KEY not in run_context.session_state:
            return "Select an analysis domain first."
        ...
        run_context.session_state[_SELECTED_CATEGORY_KEY] = chosen.value
        run_context.session_state.pop(_SELECTED_STRATEGY_KEY, None)
    ...
    def list_strategies_in_selection(
        ...
        """
        List candidate strategies that match the previously selected
        analysis domain and category. Use this to decide which strategy
        fits the user's question best.
        """
        domain_value = run_context.session_state.get(_SELECTED_DOMAIN_KEY)
        category_value = run_context.session_state.get(_SELECTED_CATEGORY_KEY)
    ...
    def select_strategy(
        ...
        Args:
            strategy_id: Full strategy id, e.g. "momentum.rsi".
        """
        if strategy_id not in self._registry.by_id:
            raise RetryAgentRun(f"Unknown strategy_id '{strategy_id}'.")

        run_context.session_state[_SELECTED_STRATEGY_KEY] = strategy_id
