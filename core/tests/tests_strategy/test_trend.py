...
def test_trend_sma_crossover(ta: TechnicalAnalysis):
    result = ta.trend.sma_crossover(fast=5, slow=10)
    cols = result.columns
...
def test_trend_ema_crossover(ta: TechnicalAnalysis):
    result = ta.trend.ema_crossover(fast=12, slow=26)
    cols = result.columns
...
def test_trend_macd(ta: TechnicalAnalysis):
    result = ta.trend.macd(fastperiod=12, slowperiod=26, signalperiod=9)
    cols = result.columns
    assert "MACD" in cols
...
def test_trend_adx_dmi(ta: TechnicalAnalysis):
    result = ta.trend.adx_dmi(period=14)
    cols = result.columns
...
def test_trend_parabolic_sar(ta: TechnicalAnalysis):
    result = ta.trend.parabolic_sar(acceleration=0.02, maximum=0.2)
    cols = result.columns
    assert "SAR" in cols
...
def test_trend_kama(ta: TechnicalAnalysis):
    result = ta.trend.kama(period=30)
    cols = result.columns
...
def test_trend_t3(ta: TechnicalAnalysis):
    result = ta.trend.t3(period=5, vfactor=0.7)
    cols = result.columns
